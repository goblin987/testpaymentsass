...
# --- NEW: Helper to Create SOL Payment for Basket ---
async def _create_sol_payment_for_basket(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Create a SOL payment for basket purchase."""
    query = update.callback_query
    user_id = query.from_user.id
    chat_id = query.message.chat_id
    lang, lang_data = _get_lang_data(context)
    ...
    basket_snapshot = context.user_data.get('basket_pay_snapshot')
    total_eur = Decimal(str(context.user_data.get('basket_pay_total_eur', 0)))
    discount_code = context.user_data.get('basket_pay_discount_code')
    
    if not basket_snapshot or total_eur <= 0:
        await query.edit_message_text("❌ Error: Payment context lost.", parse_mode=None)
        return
    
    # Show preparing message
    await query.edit_message_text("⏳ Preparing SOL payment...", parse_mode=None)
    ...
    if 'error' in payment_result:
        ...
        error_msg = "❌ Failed to create payment. Please try again."
        
        if error_code == 'price_fetch_failed':
            error_msg = "❌ Unable to fetch SOL price. Please try again in a moment."
        elif error_code == 'amount_too_low':
            min_sol = payment_result.get('min_sol', 0.01)
            min_eur = payment_result.get('min_eur', 5)
            error_msg = f"❌ Amount too low. Minimum: {min_sol} SOL (~{min_eur:.2f} EUR)"
        elif error_code == 'database_error':
            error_msg = "❌ Database error. Please contact support."
        
        back_button = InlineKeyboardButton("⬅️ Back to Basket", callback_data="view_basket")
        await query.edit_message_text(error_msg, reply_markup=InlineKeyboardMarkup([[back_button]]), parse_mode=None)
        return
    
    # Display payment instructions
    sol_amount = payment_result['sol_amount']
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⬅️ Back to Basket", callback_data="view_basket")]
    ]
    
    # Store payment ID for cancellation
    context.user_data['pending_sol_payment_id'] = payment_id
    ...
    try:
        ...
        logger.error(f"Error displaying SOL payment: {e}")
...
# --- Handler to Cancel SOL Payment ---
async def handle_cancel_sol_payment(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Cancel a pending SOL payment and unreserve items."""
    query = update.callback_query
    user_id = query.from_user.id
    lang, lang_data = _get_lang_data(context)
    
    payment_id = context.user_data.get('pending_sol_payment_id')
    ...
    # Import SOL payment module
    from sol_payment import cancel_sol_payment
    
    # Cancel the payment
    success = await cancel_sol_payment(payment_id)
    
    # Clear stored payment ID
    context.user_data.pop('pending_sol_payment_id', None)
    
    if success:
        msg = "✅ Payment cancelled successfully. Reserved items have been released."
    else:
        msg = "⚠️ Payment cancellation processed, but there may have been an issue."
    
    keyboard = [[InlineKeyboardButton("⬅️ Back to Basket", callback_data="view_basket")]]
    
    try:
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)
    except Exception as e:
        logger.warning(f"Could not edit message during payment cancellation: {e}")
        await query.answer("Payment cancelled!")
    
    await query.answer()


# --- OLD: Helper to Show Crypto Choices for Basket Payment (DEPRECATED - Replaced by SOL) ---
