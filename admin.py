...
async def handle_adm_size(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    # Go to wallet selection instead of price
    await handle_adm_wallet_selection(update, context)
...
async def handle_adm_wallet_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Admin selects which wallet receives payment for this product."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id): 
        return await query.answer("Access denied.", show_alert=True)
    
    # Get product details for display
    size = context.user_data.get("pending_drop_size")
    p_type = context.user_data.get("admin_product_type")
    city = context.user_data.get("admin_city")
    district = context.user_data.get("admin_district")
    
    if not all([size, p_type, city, district]):
        return await query.edit_message_text(
            "❌ Error: Context lost. Please start adding product again.", 
            parse_mode=None
        )
...
async def handle_adm_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Process wallet selection and ask for price."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id): 
        return await query.answer("Access denied.", show_alert=True)
    
    if not params or len(params) < 1:
        return await query.answer("Error: Wallet selection missing.", show_alert=True)
    
    wallet_choice = params[0]  # 'wallet1', 'wallet2', or 'split'
    
    # Validate wallet choice
    if wallet_choice not in ['wallet1', 'wallet2', 'split']:
        return await query.answer("Invalid wallet choice.", show_alert=True)
    ...
    msg = f"📦 Adding {type_emoji} {p_type} - {size}\n"
    msg += f"📍 {city} / {district}\n"
    msg += f"💰 Payout: {wallet_display[wallet_choice]}\n\n"
    msg += "💶 Now enter the price in EUR (e.g., 25.50):"
...
async def handle_confirm_add_drop(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        insert_params = (
            city, district, p_type, size, product_name, price, original_text, ADMIN_ID, datetime.now(timezone.utc).isoformat(), wallet_choice
        )
        logger.debug(f"Inserting product with params count: {len(insert_params)}") # Add debug log
        c.execute("""INSERT INTO products
                        (city, district, product_type, size, name, price, available, reserved, original_text, added_by, added_date, payout_wallet)
...
async def handle_adm_bulk_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    for drop in bulk_drops:
        ...
        try:
            ...
            # Get wallet choice from context (bulk products share same wallet setting)
            wallet_choice = context.user_data.get("admin_wallet_choice", "wallet1")
...
async def handle_adm_bulk_execute_messages(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    for i, message_data in enumerate(bulk_messages):
        ...
        try:
            ...
            insert_params = (
                city, district, p_type, size, product_name, price, text_content, ADMIN_ID, datetime.now(timezone.utc).isoformat(), wallet_choice
            )
